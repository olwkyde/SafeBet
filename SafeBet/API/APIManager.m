...
- (void)fetchMLBEventsWithCompletion:(void (^)(NSArray *events, NSError *error))completion {
    //get the bets from endpoint
    NSString *key = [self getAPIKey];
    NSString *endpoint = @"https://api.the-odds-api.com/v4/sports/baseball_mlb/odds/?regions=us&markets=h2h,spreads&oddsFormat=american&apiKey=";
    ...
    NSURLSessionDataTask *task = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
           if (error != nil) {
               ...
               NSArray *dataDictionary = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableContainers error:nil];
               NSMutableArray *events = [Events mlbEventsWithArray:dataDictionary];
               completion(events, nil);
           }
       }];
    [task resume];
